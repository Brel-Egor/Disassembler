...
unsigned int length_dis::length(unsigned char *code, int is64)
{
    ...
    this->opcd_offset = (unsigned char)(p - (unsigned char*)code);
    this->opcd_size = 1;
    op = *p++; s++;
    this->op_num++;
    this->masop[i] = op;
    ...
    if (f & OP_MODRM)
    {
        ...
        if (this->disp_size)
        {
            this->disp_offset = (unsigned char)(p - (unsigned char *)code);
    ...
    if (this->imm_size) {
        ...
        s += this->imm_size;
        this->imm_offset = (unsigned char)(p - (unsigned char *)code);
        this->flags |= F_IMM;
        if (f & OP_RELATIVE)
        {
            this->flags |= F_RELATIVE;
